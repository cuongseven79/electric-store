...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  if (isStaff) {
    return (
      <div className={clsx('flex h-full w-full flex-row p-2')}>
                ...
                >
                  <Link
                    ...
                    className={clsx(
                      ...
                      'flex w-full flex-row items-center justify-start rounded-full p-2 transition-all  duration-300 ease-in-out hover:bg-gray-100',
                    )}
                  >
                    <div className="text-[20px] md:w-[40px]">{link.icon}</div>
                    <div
                      className={clsx(
                        pathName.startsWith(link.href)
                          ? 'text-gray-400'
        ...
        <div className="flex h-full w-full flex-col">
          <div className="relative flex justify-between p-2">
            ...
            {isUserInfoShown && (
              <UserInfoModal handleToggle={setUserInfoShown} />
            )}
            <div className="group relative">
              <button>{<Icon icon={faCog} />}</button>
              <div className="absolute right-0 top-5 z-30 flex hidden min-h-[100px] w-max min-w-[200px] max-w-[400px] flex-col gap-6 rounded-md bg-white p-4 shadow-[0_3px_10px_rgb(0,0,0,0.2)] group-hover:flex">
  ...
  return (
    <div className="mx-auto flex min-h-[100vh] max-w-7xl flex-col items-center justify-start px-4 pt-1">
      <div className="relative mx-auto flex h-auto w-full max-w-7xl flex-col items-start justify-between py-3">
        ...
        <nav className="mx-auto flex h-auto w-full max-w-7xl flex-row items-center justify-between py-3">
          <Link
            ...
            className="flex items-center space-x-3 rtl:space-x-reverse"
          >
            <Image src={'/static/logo.jpg'} width={150} height={120} alt="" />
          </Link>
          ...
          <ul className="text-md mt-0 flex flex-row items-center justify-center gap-1 space-x-2 transition-all duration-300 ease-in-out ">
            ...
            <li className="group relative">
              ...
              >
                ...
                <div
                  className={clsx(
                    'text-[0px] group-hover:text-[14px]',
                    'text-gray-500 transition-all duration-700 ease-in-out hover:transition-all group-hover:text-[16px]',
                  )}
                >
                  Your cart
            ...
            <li className="group relative">
              ...
              <div className="absolute right-0 top-5 z-30 hidden min-h-[100px] w-max min-w-[200px] max-w-[400px] flex-col gap-6 rounded-md bg-white p-4 shadow-[0_3px_10px_rgb(0,0,0,0.2)] group-hover:flex">
                {isLogged ? (
                  <div className="flex h-full w-full flex-col gap-2">
                    <div
                      className="flex w-full items-center gap-4 hover:cursor-pointer hover:bg-gray-50"
